...
(function () {
  ...
  const mappers = {
    ...
    retiro: {
      toApi:   r => ({ tipo: r.tipo, nombre: r.nombre, institucion: r.institucion, subcuenta: r.subcuenta, saldo: r.saldo, aportacion_ytd: r.aportacionYTD || 0, aportacion_patronal: r.aportacionPatronal || 0, rendimiento: r.rendimiento, proyeccion: r.proyeccion || 0, fecha_compra: r.fechaCompra || null, fecha_retiro: r.fechaRetiro || null }),
      fromApi: r => ({ id: r.id, tipo: r.tipo, nombre: r.nombre, institucion: r.institucion, subcuenta: r.subcuenta, saldo: parseFloat(r.saldo), aportacionYTD: parseFloat(r.aportacion_ytd), aportacionPatronal: parseFloat(r.aportacion_patronal), rendimiento: parseFloat(r.rendimiento), proyeccion: parseFloat(r.proyeccion), fechaCompra: r.fecha_compra || null, fechaRetiro: r.fecha_retiro || null, history: _fakeHistory(parseFloat(r.saldo)) }),
    },
    bienes: {
  ...
  window.WOS_API = {
    ...
    // ── Auth ────────────────────────────────────────────────────────────────
    auth: {
      signup:        (email, password, firstName, lastName, dob) => request('POST', '/api/auth/signup', { email, password, first_name: firstName || '', last_name: lastName || '', date_of_birth: dob || null }),
